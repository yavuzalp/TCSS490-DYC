...
1-) I think we have done a good job over all but we faced one problem in Java code. when we start regex with ^ and end with $, it works if we put proper input at first try, 
but if we enter invalid input at first, we can't get pattern to match any input anymore. It always gives problem, so we removed ^,$ but which lets user put spaces.
